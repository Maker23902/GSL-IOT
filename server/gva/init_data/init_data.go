...
func InitSysBaseMenus() (err error) {
	...
	insert := []model.SysBaseMenu{
		...
		{Model: gorm.Model{ID: 27, CreatedAt: time.Now(), UpdatedAt: time.Now()}, MenuLevel: 0, Hidden: false, ParentId: "9", Path: "Lora", Name: "Lora", Component: "view/LoraWAN/index.vue", Sort: 8, Meta: model.Meta{Title: "Lora服务配置", Icon: "upload"}},
	}
	if tx.Create(&insert).Error != nil { // 遇到错误时回滚事务
		tx.Rollback()
	}
